...
public class BaseRedirectorEditor : Editor
{
    // predefined colors
    ...
    {
        var visualTree = Resources.Load("SpaceExtenderTool/UXML/BaseRedirector_Inspector") as VisualTreeAsset;
    ...
    protected static void DrawGizmosSelected(BaseRedirector redirector, GizmoType gizmoType)
    {
        ...
        {
            linePoints[i] = redirector.transform.TransformPoint(new Vector3(linePoints[i].x * redirector.PlayAreaDimensions.x, linePoints[i].y, linePoints[i].z * redirector.PlayAreaDimensions.y));
        }
    ...
    {
        Gizmos.DrawMesh(redirector.GizmoPlayAreaMesh, position, rotation, new Vector3(redirector.PlayAreaDimensions.x, 0f, redirector.PlayAreaDimensions.y));
    }
}
